const CACHE = 'amistmg-e09158c243';
...
self.addEventListener('fetch', e => {
  ...
  if (url.pathname.endsWith('/echeances.enc.json')){
    e.respondWith((async () => {
      ...
      try { return await revalider(e, sansCacheHttp(e.request.url), './echeances.enc.json', cache); }
      catch (_) { return (await cache.match('./echeances.enc.json')) || Response.error(); }
    })());
    return;
  }
